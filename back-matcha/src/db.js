...
async function initDatabase() {
  ...
  await Promise.all([
    ...
    pool.query(
      "CREATE TABLE IF NOT EXISTS USR_TAG ( \
  ...
  await Promise.all([
    pool.query(
      "ALTER TABLE NOTIF \
        ADD CONSTRAINT NOTIF_ibfk_1 FOREIGN KEY (USRIDACONSULT) REFERENCES USER (ID), \
        ADD CONSTRAINT NOTIF_ibfk_2 FOREIGN KEY (USRIDESTCONSULT) REFERENCES USER (ID);"
    ),
    pool.query(
      "ALTER TABLE IMG \
        ADD CONSTRAINT IMG_ibfk_1 FOREIGN KEY (USRID) REFERENCES USER (ID);"
    ),
  ]);
  */
